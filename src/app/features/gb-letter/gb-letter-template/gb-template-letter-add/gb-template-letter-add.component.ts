...
import { templateLetterStateService } from '../services/gb-letter-template-state-service'; // Service to handle letter state
...
export class GbTemplateAddComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, private letterStateService: templateLetterStateService, private router: Router) {
    ...
    this.templateForm = this.fb.group({
      ...
      step4: this.fb.group({
        stepsForChange: this.fb.array([this.createStepForChange()], Validators.required),
      }),
      step5: this.fb.group({
...
// Helper method to create a FormControl for a step
  createStepForChange(): FormControl {
    return this.fb.control('', Validators.required);
  }
  // Method to add a new step
  public addStep(): void {
    this.stepsForChange.push(this.createStepForChange());
    console.log(`Added Step ${this.stepsForChange.length}`);
    setTimeout(() => {
      const newInput = document.getElementById(`step${this.stepsForChange.length - 1}`);
      if (newInput) {
        newInput.focus();
      }
    }, 0);
  }
// Getter to access the stepsForChange FormArray
  get stepsForChange(): FormArray {
    return this.templateForm.get('step4.stepsForChange') as FormArray;
  }
...
// Method to remove a step at a specific index
  removeStep(index: number): void {
    if (this.stepsForChange.length > 1) {
      this.stepsForChange.removeAt(index);
    }
  }
